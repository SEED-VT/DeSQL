...
import org.apache.spark.sql.catalyst.plans.logical.{SubQueryExist, SubQueryVisitor}
...
case class Cast(child: Expression, dataType: DataType, timeZoneId: Option[String] = None)
  extends CastBase with SubQueryExist{
  ...
  override def accept(subQueryVisitor: SubQueryVisitor, subQueryList : ArrayBuffer[SubQueryStorage]): ArrayBuffer[SubQueryStorage] = {
    var returnedList = this.child.accept(subQueryVisitor,subQueryList)
    subQueryVisitor.visit(this,returnedList)
    //this.name
  }
}
