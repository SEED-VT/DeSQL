...
class CodegenContext extends Logging {
  ...
  var debuggerFlag = true
  var count = 1

  def mappingIndex_= (newValue: => Int): Unit = {
    mappingIndex = newValue
  }
...
object CodeGenerator extends Logging {
  ...
  private[this] def doCompile(code: CodeAndComment): (GeneratedClass, ByteCodeStats) = {
    ...
    evaluator.setDefaultImports(
      ...
      classOf[DataTypes].getName,
      classOf[Metadata].getName,
      classOf[StructType].getName,
      classOf[StructField].getName,
