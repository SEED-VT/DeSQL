...
import org.apache.spark.sql.catalyst.plans.logical.{Filter, LogicalPlan, SubQueryExist, SubQueryVisitor}
...
abstract class SubqueryExpression(
    ...
    exprId: ExprId) extends PlanExpression[LogicalPlan] with SubQueryExist{
  ...
  override def accept(subQueryVisitor: SubQueryVisitor, subQueryList : ArrayBuffer[SubQueryStorage]): ArrayBuffer[SubQueryStorage] = {
    ...
    var resultBackSubQueries = this.plan.accept(subQueryVisitor, resultSubQueries)
    subQueryList.prependAll(resultBackSubQueries)
    subQueryVisitor.visit(this,resultBackSubQueries)
  }
}
