...
abstract class SparkPlan
  ...
  /**
    ...
    */
  @transient final val sqlContext =
  ...
  def executeCollect(): Array[InternalRow] = {
    ...
    /*doExecuteere --> only rerturn an rdd that uses one UDF generated from CodeGen

    sparkContext.EnrollDebugRDD(debug_rdd, this.id)

    return rd.filte(r-=> !r.mappignIndex.isDefined)*/
    results.toArray
  }
  ...
  private def setLogicalLink(logicalPlan: LogicalPlan,
                             inherited: Boolean = false): Unit = {
    ...
    execute().mapPartitionsInternal { iter =>
      ...
      if (takeFromEnd && n > 0) {
        ...
        while (i < count) {
          ...
          out.writeInt(row.getMappingIndex)
  ...
  private def decodeUnsafeRows(bytes: Array[Byte]): Iterator[InternalRow] = {
    ...
    new Iterator[InternalRow] {
      ...
      //private var mappingIndexOfNextRow = -1
      override def hasNext: Boolean = sizeOfNextRow >= 0
      override def next(): InternalRow = {
        val bs = new Array[Byte](sizeOfNextRow)
        //mappingIndexOfNextRow = ins.readInt()
        ins.readFully(bs)
  ...
  private def executeTake(n: Int, takeFromEnd: Boolean): Array[InternalRow] = {
    ...
    while (buf.length < n && partsScanned < totalParts) {
      ...
      val res = sc.runJob(
        childRDD,
        (it: Iterator[(Long, Array[Byte])]) =>
          if (it.hasNext) it.next() else (0L, Array.emptyByteArray),
        partsToScan)
