...
import org.apache.spark.sql.execution.{BinaryExecNode, CodegenSupport, InputAdapter, SparkPlan}
...
case class BroadcastHashJoinExec(
  ...
  private def codegenInner(ctx: CodegenContext, input: Seq[ExprCode]): String = {
    ...
    var indexVal = this.getMappingIndex
    if(this.asInstanceOf[BroadcastHashJoinExec].right.isInstanceOf[InputAdapter]) {
      schemaVal = this.asInstanceOf[BroadcastHashJoinExec].right.asInstanceOf[InputAdapter].child.schema
    } else {
      schemaVal = this.asInstanceOf[BroadcastHashJoinExec].left.asInstanceOf[InputAdapter].child.schema
    }
    var codeSchemaVal = "new StructType(new StructField[] {"
    schemaVal.fields.foreach{ x =>
    {
    ...
    val tap_code = if(ctx.debuggerFlag == true) {
      s"""
         |$matched.setMappingIndex($indexVal);
         |$matched.setMappingSchema(${finalCodeSchemaVal});
         |append($matched.copy());
       """.stripMargin
    } else
        s""
