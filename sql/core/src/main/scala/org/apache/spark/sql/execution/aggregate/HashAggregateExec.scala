  ...
  private def doConsumeWithKeys(ctx: CodegenContext, input: Seq[ExprCode]): String = {
    ...
    val updateRowInRegularHashMap: String = {
      ...
      val aggCodeBlocks = updateExprs.indices.map { i =>
        ...
        val tap_code = if (this.iterVal!=0 && ctx.debuggerFlag == true) {
          s"""
             |$unsafeRowBuffer.setMappingIndex($indexVal);
             |$unsafeRowBuffer.setMappingSchema(${finalCodeSchemaVal});
             |append($unsafeRowBuffer.copy());
       """.stripMargin
        } else s""
