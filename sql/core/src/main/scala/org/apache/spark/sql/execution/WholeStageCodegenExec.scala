...
case class RowRecord(tapRow:InternalRow,id:Int, schema:StructType)

/**
 * Leaf codegen node reading from a single RDD.
 */
trait InputRDDCodegen extends CodegenSupport with Logging{
  ...
  override def doProduce(ctx: CodegenContext): String = {
    ...
    val newRow = ctx.freshName("newRow")
    ...
    var schemaVal = this.schema
    var codeSchemaVal = "new StructType(new StructField[] {"
    this.schema.fields.foreach{ x =>
    {
      codeSchemaVal = codeSchemaVal + " new StructField("
      codeSchemaVal = codeSchemaVal + "\"" + x.name + "\"" + ","
      codeSchemaVal = codeSchemaVal + "DataTypes." + x.dataType.toString + ","
      codeSchemaVal = codeSchemaVal + x.nullable.toString + ","
      codeSchemaVal = codeSchemaVal + "Metadata.empty()),"
    }
    }
    var finalCodeSchemaVal = codeSchemaVal.substring(0,codeSchemaVal.length-1)
    finalCodeSchemaVal = finalCodeSchemaVal + "})"
    var indexVal = -1
    if(this.parent.isInstanceOf[BroadcastHashJoinExec]){
      indexVal = this.asInstanceOf[InputAdapter].parent.asInstanceOf[BroadcastHashJoinExec].parent.getMappingIndex()
    }else{
      indexVal = this.parent.getMappingIndex
    }
    ...
    val instance = ctx.count

    val tap_code = if(indexVal != 0 && ctx.debuggerFlag == true) {
      s"""
         |${row}.setMappingIndex($indexVal);
         |${row}.setMappingSchema(${finalCodeSchemaVal});
         |append(${row}.copy());
       """.stripMargin
...
case class WholeStageCodegenExec(child: SparkPlan)(val codegenStageId: Int)
  extends UnaryExecNode with CodegenSupport with Logging {
  ...
  override def doExecute(): RDD[InternalRow] = {
    ...
    if (rdds.length == 1) {
      // TODO: Here add another map to tap into the records at runtime.

      var resultRDD =  rdds.head.mapPartitionsWithIndex { (index, iter) =>
      ...
      var debugRDD = resultRDD.filter(r => r.mappingIndex != 99)

      var mappedDebugRDD = debugRDD.map(record => (new GenericRowWithSchema(record.toSeq(record.getMappingSchemaStructType()).toArray,record.getMappingSchemaStructType()), "|" , record.mappingIndex,"|",record.getMappingSchema))
      this.sqlContext.sparkSession.enrollDebugBuffer(mappedDebugRDD)
      var collectRDD = resultRDD.filter(r => r.mappingIndex == 99)

      collectRDD
    } else {
      // Right now, we support up to two input RDDs.
      var resultRDD = rdds.head.zipPartitions(rdds(1)) { (leftIter, rightIter) =>
