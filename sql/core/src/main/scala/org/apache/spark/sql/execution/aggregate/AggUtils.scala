...
object AggUtils {
  ...
  private def createAggregate(
    ...
    if (useHash) {
      var newNode = HashAggregateExec(
        ...
        child = child,
        0)
        if(child.isInstanceOf[PlanLater]){
          if(child.asInstanceOf[PlanLater].plan.isInstanceOf[Project]){
            newNode.setMappingIndex(child.asInstanceOf[PlanLater].plan.asInstanceOf[Project].child.getMappingIndex)
          }else {
            newNode.setMappingIndex(child.asInstanceOf[PlanLater].plan.getMappingIndex)
          }
        }
      newNode
    } else {
  ...
  def planAggregateWithoutDistinct(
    ...
    finalAggregate.asInstanceOf[HashAggregateExec].iterVal = 1
    finalAggregate :: Nil
  }
