...
import java.io._
import java.net.URI
import org.apache.spark.sdb.SubQueryStorage
...
case class SparkSubQueryResult(subQueryString: String, rows: Array[String])
...
class SparkContext(config: SparkConf) extends Logging {
  ...
  private var _allSubQueries: Int = 0
  private var _debuggerFlag: Boolean = false
  private var _originialQuery: String = ""
  private var _basePath: String = ""
  private var planTree: String = ""

  def setPlanTree(planTree: String): Unit = {
    this.planTree = planTree
  }

  def getPlanTree: String = {
    println("Spark context start")
    println(this.planTree)
    println("Spark context end")
    this.planTree
  }


  def setAllSubQueries(totalCount: Int): Unit = {
    this._allSubQueries = totalCount
  }

  def getAllSubQueries: Int = {
    this._allSubQueries
  }

  def setDebuggerFlag(flag: Boolean): Unit = {
    this._debuggerFlag = flag
  }

  def getDebuggerFlag: Boolean = {
    this._debuggerFlag
  }

  def setOriginalQuery(query: String): Unit = {
    this._originialQuery = query
  }

  def getOriginalQuery: String = {
    this._originialQuery
  }

  def setBasePath(path: String): Unit = {
    this._basePath = path
  }
...
/*  private var debugBuffer = ArrayBuffer[InternalRow]()

  def enrollDebugBuffer(debugBufferStore: ArrayBuffer[InternalRow]): Unit = {
    debugBuffer.appendAll(debugBufferStore)
  }
  */
  /**
