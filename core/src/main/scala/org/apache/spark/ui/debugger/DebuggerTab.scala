...
//  }
  ...
  def renderPlanNodeTreeXml(planXml: scala.xml.Node, availableMappings: Set[String]): Seq[Node] = {
    ...
    val nodeName = (planXml \@ "name").toString
...
//      </div>
      ...
      val subqueriesContent = (1 to allSubqueriesCount).flatMap { fileNumber =>
        ...
        val subQueryContent: Seq[Node] =
          if (subQuery.rows.isEmpty) {
            Seq(<p>This subquery doesn't generate any data</p>)
          } else {
        ...
        Seq(
          <li data-mapping-index={mappingIndex.toString}>
    ...
    function initializeCodeMirror() {
          document.querySelectorAll('.sql-query').forEach(function (el) {
            ...
            const editor = CodeMirror.fromTextArea(el, {
              ...
              readOnly: true,
              lineWrapping: true
              // Removed viewportMargin: Infinity
            });

            // Trigger a refresh which recalculates the editor size
            editor.refresh();

            // Dynamically adjust the height based on the content after a slight delay
            setTimeout(function() {
      ...
      val style =
       ...
       .CodeMirror {
        ...
        .show-more { /* Button styles */ }
          ...
          .pretty-table, .pretty-table th, .pretty-table td {
            border: 1px solid #ddd;
            text-align: left;
            padding: 8px;
          }
      ...
      UIUtils.headerSparkPage(request, "Debugger", Seq(Unparsed(style), combinedContent, Unparsed(script)), parent, useDataTables = true)
    } else {
      ...
      UIUtils.headerSparkPage(request, "Debugger", Seq(autoRefreshScript, message), parent, useDataTables = true)
    }
  }
  ...
  private def renderRowsAsTable(rows: Seq[String], rowsId: String): String = {
    ...
    val tableRows = rows.zipWithIndex.map { case (row, index) =>
      val columns = row.split(",").map(value => s"<td>${value.trim}</td>").mkString
      val style = if (index >= 5) " class='hidden-row'" else "" // Apply class to hide rows beyond the first 5
      s"<tr$style>$columns</tr>"
    }.mkString

    val showMoreButton = if (hasMoreRows) s"""<button class="show-more" data-rows-id="$rowsId">Show More</button>""" else ""
