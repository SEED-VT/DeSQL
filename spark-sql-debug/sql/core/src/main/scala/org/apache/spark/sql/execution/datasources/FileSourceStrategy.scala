...
object FileSourceStrategy extends Strategy with Logging {
  ...
  def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
    case ScanOperation(projects, filters,
      ...
      scan.setMappingIndex(plan.getMappingIndex())
      val afterScanFilter = afterScanFilters.toSeq.reduceOption(expressions.And)
      val withFilter = afterScanFilter.map(execution.FilterExec(_, scan)).getOrElse(scan)
      if(plan.isInstanceOf[Filter] && plan.asInstanceOf[Filter].child.isInstanceOf[LogicalRelation] && plan.asInstanceOf[Filter].getMappingIndex == -1){
        withFilter.setMappingIndex(plan.asInstanceOf[Filter].child.getMappingIndex())
      } else {
        withFilter.setMappingIndex(plan.getMappingIndex())
      }
      val withProjections = if (projects == withFilter.output) {
        withFilter
      } else {
          var storeIndex = execution.ProjectExec(projects, withFilter)
          storeIndex.setMappingIndex(plan.getMappingIndex())
          storeIndex
      }
      withProjections :: Nil
