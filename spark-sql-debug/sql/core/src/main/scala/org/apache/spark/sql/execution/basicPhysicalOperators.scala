...
case class FilterExec(condition: Expression, child: SparkPlan)
  ...
  override def doConsume(ctx: CodegenContext, input: Seq[ExprCode], row: ExprCode): String = {
     ...
     */
    def genPredicate(c: Expression, in: Seq[ExprCode], attrs: Seq[Attribute], requriedSchema: StructType): String = {
      ...
      val copyC = c.toString().replace('#',' ')
      var indexVal = c.getMappingIndex

      var schemaVal = requriedSchema
      var codeSchemaVal = "new StructType(new StructField[] {"
      requriedSchema.fields.foreach{ x =>
      {
      ...
      val tap_code = if(!c.toString().contains("isnotnull") && ctx.debuggerFlag == true){
        s"""
           |if(count1%$instance == 0){
           |${row.value}.setMappingIndex($indexVal);
           |${row.value}.setMappingSchema(${finalCodeSchemaVal});
    ...
    val generated = otherPreds.map { c =>
      val nullChecks = c.references.map { r =>
        ...
        if (idx != -1 && !generatedIsNotNullChecks(idx)) {
          ...
          genPredicate(notNullPreds(idx), input, child.output,this.schema)
        } else if (notNullAttributes.contains(r.exprId) && !extraIsNotNullAttrs.contains(r)) {
          extraIsNotNullAttrs += r
          genPredicate(IsNotNull(r), input, child.output,this.schema)
        } else {
          ""
        }
      }.mkString("\n").trim
      ...
      s"""
         |$nullChecks
         |${genPredicate(c, input, output,this.schema)}
       """.stripMargin.trim
    }.mkString("\n")

    val nullChecks = notNullPreds.zipWithIndex.map { case (c, idx) =>
      if (!generatedIsNotNullChecks(idx)) {
        genPredicate(c, input, child.output,this.schema)
      } else {
        ""
      }
    }.mkString("\n")
