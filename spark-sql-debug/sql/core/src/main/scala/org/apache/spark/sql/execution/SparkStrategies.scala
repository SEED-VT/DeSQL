  ...
  object JoinSelection extends Strategy with PredicateHelper {
    ...
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      ...
      case ExtractEquiJoinKeys(joinType, leftKeys, rightKeys, condition, left, right, hint) =>
        ...
        var joinInstance = createBroadcastHashJoin(hintToBroadcastLeft(hint), hintToBroadcastRight(hint))
          .orElse { if (hintToSortMergeJoin(hint)) createSortMergeJoin() else None }
          .orElse(createShuffleHashJoin(hintToShuffleHashLeft(hint), hintToShuffleHashRight(hint)))
          .orElse { if (hintToShuffleReplicateNL(hint)) createCartesianProduct() else None }
          .getOrElse(createJoinWithoutHint())

        joinInstance(0).setMappingIndex(plan.getMappingIndex)
        joinInstance
  ...
  object Aggregation extends Strategy {
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      case PhysicalAggregation(groupingExpressions, aggExpressions, resultExpressions, child)
        ...
        aggregateOperator(0).setMappingIndex(plan.getMappingIndex)
        aggregateOperator
  ...
  object BasicOperators extends Strategy {
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      ...
      case logical.Sort(sortExprs, global, child) =>
        var storeMappingSort = execution.SortExec(sortExprs, global, planLater(child)) :: Nil
        storeMappingSort(0).setMappingIndex(plan.getMappingIndex)
        storeMappingSort
      case logical.Project(projectList, child) =>
        var storeMappingProject = execution.ProjectExec(projectList, planLater(child)) :: Nil
        storeMappingProject(0).setMappingIndex(plan.getMappingIndex)
        storeMappingProject
