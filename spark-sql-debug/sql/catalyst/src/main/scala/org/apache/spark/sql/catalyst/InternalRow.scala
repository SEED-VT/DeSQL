...
abstract class InternalRow extends SpecializedGetters with Serializable {
  ...
  var mappingSchema: String = ""

  var mappingSchemaStructType : StructType = new StructType()

  def setMappingSchema(x: StructType): Unit ={
    mappingSchemaStructType = x
    var projectionListFromSchema = ""
    x.foreach{schemaIter => {projectionListFromSchema = projectionListFromSchema + schemaIter.name + ","}}
    mappingSchema = projectionListFromSchema
  }
  def getMappingSchema(): String = {
    return mappingSchema
  }
  def getMappingSchemaStructType(): StructType = {
    return mappingSchemaStructType
  }

  var mappingIndex: Int = 99

  def setMappingIndex(x: Int): Unit ={
    mappingIndex = x
  }
