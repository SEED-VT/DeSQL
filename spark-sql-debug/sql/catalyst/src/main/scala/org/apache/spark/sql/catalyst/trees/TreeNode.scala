...
import org.apache.spark.sql.catalyst.{AliasIdentifier, IdentifierWithDatabase, InternalRow}
...
// scalastyle:on
  ...
  protected def copyTagsFrom(other: BaseType): Unit = {
    if(other.getClass().equals(this.getClass())) {
      this.setMappingIndex(other.getMappingIndex)
    }
    tags ++= other.tags
  }
  ...
  def mapChildren(f: BaseType => BaseType): BaseType = {
    if (containsChild.nonEmpty) {
      var getNode = mapChildren(f, forceCopy = false)
      getNode.setMappingIndex(this.getMappingIndex)
      getNode
    } else {
      this
    }
  ...
  private def makeCopy(
    ...
    try {
      CurrentOrigin.withOrigin(origin) {
        ...
        res.setMappingIndex(this.getMappingIndex())
  ...
  private var mappingIndex: Int = -1

  def setMappingIndex(x: => Int) = {
    mappingIndex = x
  }
  ...
  override def clone(): BaseType = {
    val a = mapChildren(_.clone(), forceCopy = true)
    a.setMappingIndex(this.getMappingIndex())
    a
  }
  ...
  lazy val allChildren: Set[TreeNode[_]] = (children ++ innerChildren).toSet[TreeNode[_]]
  def allChildrenFunc(): Set[TreeNode[_]] = allChildren
