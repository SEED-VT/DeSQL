    ...
    private def removeTopLevelSort(plan: LogicalPlan): LogicalPlan = {
      plan match {
        ...
        case Project(fields, child) =>
          var getProjectInstance = Project(fields, removeTopLevelSort(child))
          getProjectInstance.setMappingIndex(plan.getMappingIndex())
          getProjectInstance
        case other => other
      }
    }
  ...
  private def removeRedundantAliases(plan: LogicalPlan, blacklist: AttributeSet): LogicalPlan = {
    plan match {
      ...
      case Join(left, right, joinType, condition, hint) =>
        ...
        var getMappingInstance = Join(newLeft, newRight, joinType, newCondition, hint)
        getMappingInstance.setMappingIndex(plan.getMappingIndex)
        getMappingInstance
  ...
  def apply(plan: LogicalPlan): LogicalPlan = plan transform {
    ...
    case p @ Project(_, child) if child.sameOutput(p) =>
      if(!child.isInstanceOf[Join]) {
        child.setMappingIndex(plan.getMappingIndex())
      }
      child
  ...
  private def inferFilters(plan: LogicalPlan): LogicalPlan = plan transform {
    case filter @ Filter(condition, child) =>
      ...
      if (newFilters.nonEmpty) {
        var getMappingInstance = Filter(And(newFilters.reduce(And), condition), child)
        getMappingInstance.setMappingIndex(plan.getMappingIndex)
        getMappingInstance
      } else {
        filter
...
object PushPredicateThroughJoin extends Rule[LogicalPlan] with PredicateHelper {
  ...
  val applyLocally: PartialFunction[LogicalPlan, LogicalPlan] = {
    ...
    case f @ Filter(filterCondition, Join(left, right, joinType, joinCondition, hint)) =>
      var mappingIndexForJoin = f.child.getMappingIndex()
      ...
      joinType match {
        case _: InnerLike =>
          ...
          join.setMappingIndex(mappingIndexForJoin)
          if (others.nonEmpty) {
        ...
        case RightOuter =>
          ...
          newJoin.setMappingIndex(mappingIndexForJoin)
